...
export const Lista = ({ pets, especie }: ListaProps) => {
    ...
    const [idAux, setIdAux] = useState<number>(0);
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/deletePet`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ deleteId, especie: especie }),
            });
            if (res.ok) {
                setAlert('Registro eliminado con éxito.');
                setTimeout(() => {
                    setAlert('')
                }, 3000);
                ...
                setDeleteId(0);
                setAlert('Error al eliminar el registro, inténtelo más tarde.');
                setTimeout(() => {
                    setAlert('');
    ...
    return (
        <div className="bg-gray-100 relative shadow-md sm:rounded-lg overflow-hidden pt-5 flex flex-col m-auto">
            ...
            <div className="relative overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-900 font-itim">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-200">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3">Personalidad</th>
                            {especie === 'perro' ?
                    ...
                    {mascotas.length > 0 ? (
                        <tbody>
                            {mascotas.map((mascota) => (
                                <tr key={mascota.id} className="bg-white border-b">
                                    ...
                                    {especie === 'perro' ?
                                        <td className="px-6 py-4 text-gray-700">{mascota.tamanio}</td>
                                        : null
                                    }
                                    <td className="px-6 py-4 text-gray-700">
                                    ...
                                    <td className="px-6 py-4 text-gray-700">
                                        <div className="flex items-center gap-1">
                                            <Link className="p-2  rounded-full  group transition-all duration-500  flex item-center" href={`/admin/formPets/${especie}/${mascota.id}`}>
                                            ...
                                            <div className="p-2 rounded-full group transition-all duration-500  flex item-center cursor-pointer" onClick={() => { setConfirmDelete(true); setIdAux(mascota.id) }}>
                        ...
                        <tbody>     
                            <tr>
                                <td colSpan={especie === 'perro' ? 8 : 7} className="px-6 py-4 text-center font-bold text-xl text-gray-700 h-24 flex items-center justify-center">
                                    No hay mascotas
                                </td>
                            </tr>
                        </tbody>
            ...
            {confirmDelete && (
                <div className="absolute top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white p-5 rounded-lg font-itim">
                        <p>¿Estás seguro que deseas eliminar esta mascota?</p>
                        <div className="flex items-center justify-center gap-4 mt-5">
                            <button className="bg-red-600 text-white px-3 py-1 rounded-md" onClick={() => { setConfirmDelete(false) }}>Cancelar</button>
                            <button className="bg-green-600 text-white px-3 py-1 rounded-md" onClick={() => { setDeleteId(idAux) }}>Aceptar</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="absolute top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white p-5 rounded-lg font-itim">
                        <div className="flex items-center justify-center">
                            <div className="bg-red-600 text-white px-3 py-1 rounded-md">{alert}</div>
                        </div>
                    </div>
                </div>
            }
