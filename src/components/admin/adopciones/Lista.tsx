...
export const Lista = ({ pets, especie }: ListaProps) => {
    ...
    const deletePet = async () => {
        const res = await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/api/deletePet/${especie}/${idAux}`);
        if (res.status === 200) {
            setMascotas((prev) => prev.filter((pet) => pet.id !== idAux));
    ...
    return (
        <div className="w-full h-full">
            {success &&
                <Exito mensaje={'Perrito eliminado con éxito'} />
            }
            <div className="bg-gray-100 relative shadow-md sm:rounded-lg overflow-hidden pt-5 flex flex-col m-auto w-full">
                <div className="flex items-center justify-end px-6 mb-5 gap-4">
                    <Link href={`/admin/insertPet/${especie}`}>
                                ...
                                {mascotas.map((mascota) => (
                                    <tr key={mascota.id} className="bg-white border-b">
                                        ...
                                        <td className="py-4 text-gray-700">
                                            <Image src={mascota.imagen_principal as string} alt={mascota.nombre as string} width={100} height={100} className='rounded-lg' />
                                        </td>
                                        <td className="px-6 py-4 text-gray-700">
                                            <div className="flex items-center gap-1">
                                                <Link className="p-2  rounded-full  group transition-all duration-500  flex item-center" href={`/admin/editPet/${especie}/${mascota.id}`}>
            ...
            {confirmDelete && (
                <div className="absolute top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white p-4 rounded-md shadow-md">
                        <p className="text-center">¿Estás seguro de que deseas eliminar a este {especie === 'perros' ? 'perrito' : 'gatito'}?</p>
